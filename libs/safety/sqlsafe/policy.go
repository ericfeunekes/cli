...
func DefaultPolicy() Policy {
	...
	block := func(keyword string) RuleFunc {
		return func(_ *Policy, _ Statement, _ []string) Decision {
			return Decision{
				Allow:          false,
				Reason:         fmt.Sprintf("blocked by read-only policy: keyword %s", keyword),
...
func (p *Policy) decide(stmt Statement, tokens []string) Decision {
	...
	return Decision{Allow: false, Reason: fmt.Sprintf("blocked by read-only policy: keyword %s", keyword), BlockedKeyword: keyword}
}
